...
# E.g., for generic model `models[A, B]`, when parametrized model `models[int, str]` is created,
# `models[int, str]`: {A: int, B: str}` will be stored in `_assigned_parameters`.
...
class GenericModel(BaseModel):
    ...
    def __parameterized_bases__(cls, typevars_map: Parametrization) -> Iterator[Type[Any]]:
        ...
        :param typevars_map: Dictionary of type applications for binding subclasses.
            Given a generic class `models` with 2 type variables [S, T]
            and a concrete model `models[str, int]`,
